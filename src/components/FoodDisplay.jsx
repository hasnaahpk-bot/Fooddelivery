...
const FoodDisplay = () => {
  const { food_list, cart, addToCart, removeFromCart, searchQuery } = useContext(StoreContext);
  ...
  return (
    <div id="menu" className="px-6 md:px-16 py-10">
      ...
      {filteredList.length > 0 ? (
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center mt-3">
                  ...
                  {cart[item._id] ? (
                    ...
                    <button
                      onClick={() => addToCart(item._id)}
                      className="bg-orange-500 text-white rounded-full w-8 h-8 flex justify-center items-center text-xl hover:bg-orange-600"
                    >
                      +
                    </button>
                  )}
        ...
        <p className="text-gray-500">No food items found for your search.</p>
      )}
    </div>
  );
};
